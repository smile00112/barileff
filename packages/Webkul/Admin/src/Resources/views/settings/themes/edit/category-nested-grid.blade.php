<v-category-nested-grid :errors="errors">
    <x-admin::shimmer.settings.themes.category-carousel />
</v-category-nested-grid>

@pushOnce('scripts')
    <script
        type="text/x-template"
        id="v-category-nested-grid-template"
    >
        <div class="flex flex-1 flex-col gap-2 max-xl:flex-auto">
            <div class="box-shadow rounded bg-white p-4 dark:bg-gray-900">
                <div class="mb-2.5 flex items-center justify-between gap-x-2.5">
                    <div class="flex flex-col gap-1">
                        <p class="text-base font-semibold text-gray-800 dark:text-white">
                            @lang('admin::app.settings.themes.edit.category-nested-grid')
                        </p>

                        <p class="text-xs font-medium text-gray-500 dark:text-gray-300">
                            @lang('admin::app.settings.themes.edit.category-nested-grid-description')
                        </p>
                    </div>
                </div>
                ...
                <x-admin::form.control-group>
                    <x-admin::form.control-group.label class="required">
                        @lang('admin::app.settings.themes.edit.desktop-columns')
                    </x-admin::form.control-group.label>

                    <v-field
                        type="number"
                        name="{{ $currentLocale->code }}[options][desktop_columns]"
                        value="{{ $theme->translate($currentLocale->code)->options['desktop_columns'] ?? 4 }}"
                        class="flex min-h-[39px] w-full rounded-md border px-3 py-2 text-sm text-gray-600 transition-all hover:border-gray-400 focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300 dark:hover:border-gray-400 dark:focus:border-gray-400"
                        :class="[errors['{{ $currentLocale->code }}[options][desktop_columns]'] ? 'border border-red-600 hover:border-red-600' : '']"
                        rules="required|integer|min_value:1|max_value:6"
                        label="@lang('admin::app.settings.themes.edit.desktop-columns')"
                        placeholder="@lang('admin::app.settings.themes.edit.desktop-columns')"
                    >
                    </v-field>

                    <x-admin::form.control-group.error control-name="{{ $currentLocale->code }}[options][desktop_columns]" />
                </x-admin::form.control-group>

                <!-- Mobile Columns -->
                <x-admin::form.control-group>
                    <x-admin::form.control-group.label class="required">
                        @lang('admin::app.settings.themes.edit.mobile-columns')
                    </x-admin::form.control-group.label>

                    <v-field
                        type="number"
                        name="{{ $currentLocale->code }}[options][mobile_columns]"
                        value="{{ $theme->translate($currentLocale->code)->options['mobile_columns'] ?? 2 }}"
                        class="flex min-h-[39px] w-full rounded-md border px-3 py-2 text-sm text-gray-600 transition-all hover:border-gray-400 focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300 dark:hover:border-gray-400 dark:focus:border-gray-400"
                        :class="[errors['{{ $currentLocale->code }}[options][mobile_columns]'] ? 'border border-red-600 hover:border-red-600' : '']"
                        rules="required|integer|min_value:1|max_value:4"
                        label="@lang('admin::app.settings.themes.edit.mobile-columns')"
                        placeholder="@lang('admin::app.settings.themes.edit.mobile-columns')"
                    >
                    </v-field>

                    <x-admin::form.control-group.error control-name="{{ $currentLocale->code }}[options][mobile_columns]" />
                </x-admin::form.control-group>

                <!-- Show Name -->
                <x-admin::form.control-group>
                    <x-admin::form.control-group.label class="required">
                        @lang('admin::app.settings.themes.edit.show-category-name')
                    </x-admin::form.control-group.label>

                    <v-field
                        name="{{ $currentLocale->code }}[options][show_name]"
                        value="{{ (string) ($theme->translate($currentLocale->code)->options['show_name'] ?? '1') }}"
                        v-slot="{ field }"
                        rules="required"
                        label="@lang('admin::app.settings.themes.edit.show-category-name')"
                    >
                        <select
                            name="{{ $currentLocale->code }}[options][show_name]"
                            v-bind="field"
                            class="custom-select flex min-h-[39px] w-full rounded-md border bg-white px-3 py-1.5 text-sm font-normal text-gray-600 transition-all hover:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300"
                            :class="[errors['{{ $currentLocale->code }}[options][show_name]'] ? 'border border-red-600 hover:border-red-600' : '']"
                        >
                            <option value="1">
                                @lang('admin::app.settings.themes.edit.yes')
                            </option>

                            <option value="0">
                                @lang('admin::app.settings.themes.edit.no')
                            </option>
                        </select>
                    </v-field>

                    <x-admin::form.control-group.error control-name="{{ $currentLocale->code }}[options][show_name]" />
                </x-admin::form.control-group>
                ...
                <div class="flex items-center justify-between gap-x-2.5">
                    ...
                    <div class="flex gap-2.5">
                        <div
                            ...
                            @click="$refs.categoryNestedGridFilterModal.toggle()"
                        >
                ...
                >
                    <img
                        ...
                        alt="@lang('admin::app.settings.themes.edit.category-nested-grid')"
                    >
            ...
            >
                <form @submit="handleSubmit($event, addFilter)">
                    <x-admin::modal ref="categoryNestedGridFilterModal">
                        <x-slot:header>
    ...
    <script type="module">
        app.component('v-category-nested-grid', {
            template: '#v-category-nested-grid-template',
            ...
            methods: {
                addFilter(params) {
                    ...
                    this.$refs.categoryNestedGridFilterModal.toggle();
                },
